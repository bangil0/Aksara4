...
class Core extends Controller
{
	...
	public function render($table = null, $view = null)
	{
		...
		else
		{
			...
			elseif($this->_api_request && in_array($this->_method, array('create', 'update')) && 'POST' == env('REQUEST_METHOD'))
			{
				/**
	...
	private function _handshake($api_key = 0)
	{
		...
		elseif(!in_array(env('REQUEST_METHOD'), json_decode($client->method, true)))
		{
	...
	private function _push_log()
	{
		...
		$source										= @file_get_contents(WRITEPATH . 'logs' . '/' . $filename);
		...
		if(!is_dir(WRITEPATH . 'logs'))
		{
			...
			if(@mkdir(WRITEPATH . 'logs', 0755, true) && file_put_contents(WRITEPATH . 'logs' . '/' . $filename, json_encode($source)))
			{
				// log recorded
			}
		}
		...
		elseif(is_writable(WRITEPATH . 'logs') && file_put_contents(WRITEPATH . 'logs' . '/' . $filename, json_encode($source)))
		{
			// log recorded
		}
	}
	...
	private function _set_language($language_id = '')
	{
		...
		{
			$locale									= (service('request')->getServer('HTTP_ACCEPT_LANGUAGE') ? explode(',', service('request')->getServer('HTTP_ACCEPT_LANGUAGE')) : 'en-us');
